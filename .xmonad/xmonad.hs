...
-- like cylons, except x windows

-- pseudo conditional key bindings
-- (for windowGo)
...
-- for popOldestHiddenWindow

-- Have window deco topbar the whole window size
-- Check screen width & adjust layouts

-- Resizable Horizontal border

-- Layouts inside windows. Excellent.

-- myManageHookShift

-- import Data.Default
-- to get my old key bindings working
-- don't just hard quit
-- for the capture window

-- removeKeys, additionalKeys

-- testing
-- for spawnPipe and hPutStrLn
-- custom WS functions filtering NSP

import Control.Monad (join, liftM, liftM2) -- myManageHookShift
...
import XMonad.StackSet (shiftMaster, swapDown, swapUp)
...
projects =
  [ Project
      { projectName = wsOne,
        projectDirectory = "~/",
        projectStartHook = Nothing -- Just $ do
        -- spawnOn wsOne "alacritty"
      },
    Project
      { projectName = wsCode,
        projectDirectory = "~/",
        projectStartHook = Just $ do
          spawnOn wsCode myTmuxTerminal
          spawnOn wsCode "xclock"
          spawnOn wsCode "xeyes"
...
myLemonbarPP =
  defaultPP
    { ppTitle = ("WIN" ++),
      ...
      ppSep = "\n",
      ppWsSep = " ",
      ppOutput = \x -> writeFile "/tmp/xmonad_lemonbar_mb" $ "WSP" ++ x ++ "\n"
    }

-- The XPConfig definition for my XMonad.Prompts
myXPConfig =
  defaultXPConfig
    { bgColor = "#000030", -- dark dark blue
      fgColor = "white",
      borderColor = "#000060",
      position = Top,
...
-- myTerminal = "urxvt"
myTerminal = "/home/mb/bin/terminal"

myTmuxTerminal = "/home/mb/bin/terminal -e tmux attach-session -t work"
...
allImCommand = "dex $HOME/.local/share/applications/ferdi.desktop"

allImInfix = "allIm"

allImResource = "ferdi"

isAllIm = (resource =? allImResource)
...
scratchpads =
  [ (NS "im" imCommand isIm defaultFloating),
    (NS "allIm" allImCommand isAllIm defaultFloating),
...
shiftAndView dir =
  findWorkspace getSortByIndex dir (WSIs notSP) 1
...
myKeys' conf =
   ...
   in -----------------------------------------------------------------------
      ...
      subKeys
        ...
        [ -- ("M-<Space>"              , addName "Launcher"                        $ spawn myLauncher)
          ...
          ("M-m", addName "NSP allIm" $ namedScratchpadAction scratchpads "allIm"),
        ...
        subKeys
          ...
          [ ("M-q", addName "Restart XMonad" $ spawn "xmonad --restart"),
            ...
            -- , ("M-c"          , addName "Capture a thought"         $ inputPrompt myXPConfig "Capture" ?+ (\x -> spawn ("/home/mb/bin/capture.sh " ++ x)))
            ...
            ("M-S-l", addName "Lock the Screen" $ spawn "/home/mb/bin/lock"),
  ...
  where
    toggleCopyToAll =
      wsContainingCopies >>= \ws -> case ws of
        [] -> windows copyToAll
