     ...
     (use-package org-capture
       ...
       (setq org-capture-templates '(("i" "Item [inbox]" entry
                                     ...
                                     ("T" "Timed" entry
                                      ...
                                      "* %i%? \n %U")
                                     ("l" "Link" entry
                                      (file+headline "~/Dropbox/Apps/SimpleTxtEditor/Notizen.org" "Readit online")
                                      "* %a %^g\n %?\n %T\n %i")
                                     ("w" "Url [inbox]" entry
                                      (file "~/Org/inbox.org")
...
#+BEGIN_SRC emacs-lisp
  ...
  ;;       browse-url-generic-program "conky")
  (setq browse-url-browser-function 'browse-url-generic
...
#+BEGIN_SRC emacs-lisp
  (use-package mu4e
    ...
    ;; tell message-mode ohow to send mail
    (setq user-mail-address "mbrettschneider@heilmannsoftware.de")
    ...
    ;;              '("((date:7d..now) AND not (maildir:/gmail/[Gmail].Trash OR maildir:/gmail/[Gmail].Spam OR maildir:\"/gmail/[Gmail].All Mail\") AND not flag:trashed)" "No Trash" ?b))
    (add-to-list 'mu4e-bookmarks
                 '("(((date:30d..now) AND not flag:trashed) AND ((maildir:/gmail/INBOX) OR (maildir:/gmail-priv/INBOX) OR (maildir:/frosch03/Inbox)))" "Inbox" ?i))
...
:ensure t
:config
(setq hyde/hyde-list-posts-command "ls -ltr *.md"
...
#+begin_src emacs-lisp
  ...
  (use-package lsp-haskell
    :ensure t
    :config
    (setq lsp-haskell-server-path "/home/mb/.cabal/bin/haskell-language-server")
...
#+begin_src emacs-lisp
  (defun frog-change-column-width ()
    (setq fill-column 120))
  (add-hook 'change-log-mode-hook 'frog-change-column-width)
...
#+begin_src emacs-lisp
  (use-package imenu-list
    ...
    (setq imenu-list-after-jump-hook nil) ; Recenter curser after jumping from imenu to its entry\
    )
#+end_src
...
#+begin_src emacs-lisp
  (use-package fixme-mode
    ...
    :config
    (setq fixme-highlighted-words '("FIXME" "TODO" "BUG" "HACK"))
